...
// **** ETHERNET SETTING ****
//#define ALARM_NANO 0x02
#define ALARM_UNO  0x03

byte mac[] = { 0xCA, 0xFE, 0xBA, 0xBE, 0x00, ALARM_UNO };
int port = 12900;
...
//StateType previousDetect = state;
...
void setup() {
	...
	#define OUT_Z1      4
	#define OUT_Z2      3
	#define IN_PUSH		9	// #2 seems reserved (for ENC28J60 based ethernet shield?)
...
//		}
//	}
//	previousDetect = thisDetect;

#ifndef ALARM_ANALOG
	if (digitalRead(IN_ALARM)) {
		// Alarm state is inverted through BC547 transistor
		alarmState = ALARM_STATE_NULL;
		alarmStateString = "alarm_state_null";
	} else {
		alarmState = ALARM_STATE_ACTIVE;
		alarmStateString = "alarm_state_active";
	}
#endif
}
...
#else
	int alarm = digitalRead(IN_ALARM);
#endif
	int door = digitalRead(IN_PUSH);

	char str[50];
	sprintf(str,  "armed:%d shell:%d disa:%d alarm:%d door:%d", armed, shell, disa, alarm, door);
	client.println(str);
	Serial.println(str);
}
